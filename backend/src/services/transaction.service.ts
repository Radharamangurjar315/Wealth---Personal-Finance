...
export const scanReceiptService = async (file: Express.Multer.File | undefined) => {
  ...
  try {
    ...
    // Smart extraction — very simple parsing
    const amountMatch = text.match(/\b(\d{2,6})(\.\d{1,2})?\b/);
    ...
    return { error: "Failed to scan receipt. Try a clearer image." };
  }
};
